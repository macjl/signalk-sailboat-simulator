...
function buildSchema () {
  return {
    ...
    description: 'Simulates a virtual sailboat from autopilot intent, polar performance, weather and optional shore-distance data already present in Signal K.',
    properties: {
      initialState: {
        type: 'object',
        title: 'Initial state',
      ...
      wind: {
        type: 'object',
        title: 'Wind',
        description: 'What it does: reads wind at the simulated boat position from the Signal K Weather API and publishes the selected wind values. The main goal is to feed polar performance calculations so another plugin can produce performance.polarSpeed, which the simulator then uses as boat speed. Requires: a Weather API provider, preferably @signalk/open-meteo-provider, and a polar performance plugin such as signalk-polar-performance-plugin. Options: publish true wind, publish apparent wind, optionally force a provider id instead of the Signal K default, and choose the polling interval.',
        properties: {
          trueWind: {
            type: 'boolean',
            title: 'Publish true wind',
            description: 'Publishes environment.wind.speedTrue, environment.wind.directionTrue and environment.wind.angleTrueWater. This is usually needed by polar performance plugins.',
            default: true
          },
          apparentWind: {
            type: 'boolean',
          ...
          pollIntervalSeconds: {
            ...
            description: 'How often the simulator refreshes wind from the Weather API while wind publishing is enabled.',
            default: 60,
            minimum: 30
          }
        }
      ...
      publishing: {
        ...
        description: 'What it does: publishes the simulated boat navigation state. Requires: steering.autopilot target data to steer and performance.polarSpeed to move. Recommended providers: @signalk/signalk-autopilot, signalk-polar-performance-plugin and signalk-derived-data with navigation.magneticVariation publishing enabled for magnetic heading handling.',
        properties: {
          navigation: {
            type: 'boolean',
            title: 'Publish navigation state',
            description: 'Publishes navigation.position, navigation.headingTrue, navigation.headingMagnetic when available, navigation.courseOverGroundTrue, navigation.speedOverGround and navigation.speedThroughWater.',
            default: true
          }
        }
      },
